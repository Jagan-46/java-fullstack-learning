...
public class StudentController {
    ...
    public Page<StudentDTO> getAllStudents(@RequestParam(defaultValue = "0") int page,@RequestParam(defaultValue = "10") int size,@RequestParam(defaultValue = "id") String sortBy,@RequestParam (defaultValue = "asc") String sortDirection){
        String[] validFields = {"id","name","marks"};
        boolean isValid = false;
        for(String valid : validFields){
            if(sortBy.equalsIgnoreCase(valid)) {
                isValid = true;
                break;
            }
        }
        ...
        return students.map(student -> studentService.convertToDto(student));
    }
    ...
    public StudentDTO searchById(@PathVariable int id) {
        logger.info("Search Request for Id " + id);
